...
const createRental = async (req, res, next) => {
  try {
    const schema = Joi.object({
      ...
      cnpj: Joi.string().trim().regex(getCnpjPattern()).required().messages({
      ...
      endereco: Joi.array()
        .items(
          Joi.object({
            cep: Joi.string().trim().regex(getCepPattern()).required().messages({
            ...
            isFilial: Joi.boolean().required().messages({
